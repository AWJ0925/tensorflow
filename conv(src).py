...
"""Contains the convolutional layer classes and their functional aliases-别名.
"""
...
# 该层创建一个卷积核与该层输入进行卷积，从而产生一个输出
class _Conv(base.Layer):
    """Abstract nD convolution layer (private, used as implementation base用作实现基础).
    ...
    Arguments: # 参数
                    数据类型：
        rank(等级): An integer, the rank of the convolution, e.g. "2" for 2D convolution.
        ...
        padding: One of `"valid"` or `"same"` (case-insensitive:不区分大小写).
        data_format(数据格式): A string, one of `channels_last` (default) or `channels_first`.
        ...
        *dilation_rate(膨胀率): An integer or tuple/list of n integers, specifying the dilation rate to use for dilated convolution.
        ...
        name: A string, the name of the layer(层名).
    """
    ...
    def build(self, input_shape):  # 设置一些参数
        input_shape = tensor_shape.TensorShape(input_shape)
        if self.data_format == 'channels_first':
            channel_axis = 1  # 第二个维度
        else:
            channel_axis = -1  # 最后一个维度
    ...
    def _compute_output_shape(self, input_shape):  # 计算输出shape
...
# Concrete layer
class Conv1D(_Conv):
    """1D convolution layer (e.g. temporal convolution-时间卷积).
...
class Conv2D(_Conv):
    """2D convolution layer (e.g. spatial convolution over images-图像上的空间卷积).
